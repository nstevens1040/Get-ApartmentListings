function Get-ApartmentListings_WalkScore
{
    ...
    foreach($listing in $JSON.results)
    {
        ...
        $item.Address = $listing[16] + ", $(Get-FullAddress -Lat "$($listing[1])" -Lng "$($listing[2])")"
        ...
        $item.ListingPrice = [System.Convert]::ToInt32([regex]::New(",").Replace($listing[22][2],[string]::Empty))
        $item.Price_High = $listing[4][-1]
        $item.Price_Low = $listing[4][0]
        $item.Link = "https://www.walkscore.com/score/" + $listing[10]
        $collection.Add($item)
        $i++
        write-progress -PercentComplete ($i/$all*100) -Status "$([Math]::Round(($i/$all*100),2))%" -Activity "WalkScore :: $($i) of $($all) listings collected"
    }
    return $collection
}
